...
export async function loadTodayBoard(workspace: Workspace): Promise<{
  ...
  const projects = await listTodayProjects(workspace);
  const today = tokyoTodayIso();
  const ids = projects.map((item) => item.projectId);
  if (ids.length === 0) {
    const pending = await countPendingCaptures(workspace);
    return {
      projects: [],
      ops: {
        photoCount: 0,
  ...
  const supabase = await createServerSupabaseClient();
  const [photos, reports, tasks, processes] = await Promise.all([
    supabase
      .from("photos")
